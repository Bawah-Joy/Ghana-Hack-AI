    ...
    <View style={[styles.container, { backgroundColor: isDark ? '#111827' : '#f8fafc' }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={clearHistory} disabled={history.length === 0}>
          <Text style={[styles.clearButton, { color: history.length === 0 ? (isDark ? '#4b5563' : '#9ca3af') : '#ef4444' }]}>
            Clear
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: isDark ? '#fff' : '#000' }]}>Loading history...</Text>
        </View>
      ) : history.length === 0 ? (
        <View style={styles.emptyContainer}>
          <FontAwesome name="history" size={48} color={isDark ? '#4b5563' : '#d1d5db'} />
